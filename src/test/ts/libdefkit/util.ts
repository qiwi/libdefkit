...
describe('util', () => {
  describe('#invoke', () => {
    ...
    const cases: [string, ICmdInvokeOptions, string?, (string | null)?, any?][] = [
      [
        'uses global cmd ref',
        {cmd: 'tsc'},
        'tsc',
    ...
    cases.forEach(([description, opts, expectedCmd, expectedResult, expectedError]) => {
      it(description, () => {
        const expectedArgs = formatArgs(opts.args)
        const expectedOpts = {cwd: opts.cwd || process.cwd(), stdio: opts.stdio || STDIO_INHERIT}
